...
import { 
    ...
    Filter, X, StickyNote, RefreshCw, Send, Clock, Calendar as CalendarIcon, Zap, Upload
} from 'lucide-react';
...
const Students = () => {
    ...
    const handleOpenCreate = () => {
        if (!isPro && students.length >= 5) {
            showToast.error('Límite de 5 alumnos alcanzado en versión Gratis. ¡Pasate a PRO para ilimitados!');
            return;
        }
        resetForm();
        setFormStep(1);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditing && editingId) {
                ...
                // Check 5-student limit for Free users
                if (!isPro && students.length >= 5) {
                    showToast.error('Límite de 5 alumnos alcanzado. Pasate a PRO para ilimitados.');
                    return;
                }
    ...
    const handleExcelImport = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (!isPro) {
            showToast.error('La importación desde Excel es una función de Cobralo PRO');
            return;
        }
        ...
        reader.onload = async (evt) => {
            try {
                ...
                showToast.loading(`Importando ${data.length} alumnos...`);
                
                let successCount = 0;
                for (const row of data) {
                    // Basic mapping attempt
                    const studentData = {
                        name: row.Nombre || row.name || row.Alumno,
                        phone: (row.Telefono || row.phone || row.WhatsApp || '').toString(),
                        service_name: row.Servicio || row.service || user?.defaultService || 'General',
                        price_per_hour: Number(row.Precio || row.price || row.Cuota) || user?.defaultPrice || 0,
                        classes_per_month: 4,
                        payment_method: 'Efectivo',
                    ...
                    if (studentData.name) {
                        await api.createStudent(studentData as any);
                        successCount++;
                    }
                }

                showToast.success(`¡Se importaron ${successCount} alumnos con éxito!`);
                loadStudents();
            } catch (err) {
                console.error("Import error:", err);
                showToast.error('Error al procesar el archivo. Asegurate que sea .xlsx o .csv');
            }
        };
    ...
    const exportToCSV = () => {
        if (!isPro) {
            showToast.error('La exportación Excel/CSV es una función de Cobralo PRO');
            return;
        }
    ...
    return (
        <Layout>
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
                <div>
                    <h1 className="text-3xl font-extrabold text-zinc-900 dark:text-white tracking-tight">Gestión de Alumnos</h1>
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    >
                        ...
                        {!isPro && <Zap size={10} className="text-amber-500 absolute -top-1 -right-1 fill-amber-500" />}
                    </button>
                    <div className="relative">
                        ...
                        <button
                            onClick={() => {
                                if (!isPro) {
                                    showToast.error('La importación desde Excel es una función PRO');
                                    return;
                                }
            ...
            <div className="flex flex-col lg:flex-row gap-4 mb-6">
                <div className="relative flex-1 max-w-md">
                    ...
                    <input
                        ...
                        className="w-full pl-12 pr-4 py-3 bg-white dark:bg-bg-soft dark:text-white border border-zinc-100 dark:border-border-emerald rounded-xl outline-none focus:ring-2 focus:ring-primary-main/20 shadow-sm transition"
                    />
                </div>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <select
                        ...
                        className="px-3 py-2 bg-white dark:bg-bg-soft dark:text-white rounded-lg border border-zinc-100 dark:border-border-emerald text-sm outline-none font-bold uppercase text-[10px] tracking-widest"
                    >
            ...
            <div className="card-premium overflow-hidden">
                <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left">
                        <thead className="bg-zinc-50 dark:bg-bg-dark border-b border-zinc-100 dark:border-border-emerald">
                            <tr>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={6} className="p-10 text-center text-zinc-400">No se encontraron alumnos</td></tr>
                            ) : filteredStudents.map(student => (
                                <tr key={student.id} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-bg-dark transition">
                                    <td className="p-4">
                                        <div className="flex flex-col">
                                            <p className="font-bold text-zinc-800 dark:text-white leading-tight">{student.name}</p>
                                            <p className="label-premium !tracking-widest mt-0.5">{student.phone}</p>
                                        </div>
                                    </td>
                                    <td className="p-4">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">{student.service_name}</span>
