    ...
    <nav className={`transition-all duration-300 ${isScrolled ? 'bg-black/90 backdrop-blur-2xl py-2 shadow-2xl shadow-primary/5 border-b border-primary/10' : 'bg-black/0 backdrop-blur-none py-4 border-b border-transparent'}`}>
      <div className="container flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link to="/app/login" className="text-sm font-medium text-text-muted hover:text-text transition-colors flex items-center gap-2">
          ...
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
          >
            <Link to="/app/login?register=true" className="btn btn-primary text-xs !py-3">
              Empezar gratis
            </Link>
          </motion.div>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <Link to="/app/login" onClick={() => setIsOpen(false)} className="text-lg font-medium text-text-muted flex items-center gap-2">
              <LogIn size={20} /> Ingresar
            </Link>
            <Link to="/app/login?register=true" onClick={() => setIsOpen(false)} className="btn btn-primary w-full justify-center">Empezar gratis</Link>
          </motion.div>
        )}
      </AnimatePresence>
    </nav>
