...
async function main() {
  ...
  const yogaTeacher = await prisma.user.upsert({
    ...
    create: {
      ...
      biography: 'Instructora certificada con más de 10 años de experiencia en Hatha y Vinyasa.',
      city: 'CABA',
  ...
  const musicTeacher = await prisma.user.upsert({
    ...
    create: {
      ...
      biography: 'Músico profesional. Clases personalizadas de guitarra criolla y eléctrica.',
      city: 'Córdoba',
  ...
  const techTeacher = await prisma.user.upsert({
    ...
    create: {
      ...
      biography: 'Soporte técnico integral y armado de PCs de alto rendimiento.',
      city: 'Rosario',
